...
contract CryptoAnimal is CryptoAnimalAuction {
  event hatched(address tokenOwner, uint tokenId);

  event newEgg(address tokenOwner, uint tokenId, uint partnerId, uint eggId);
  ...
  {    
    ...
    require(animal.time.eggTime + hatchReadyTime < block.timestamp, "Animal: not ready to hatch");
    require(hatchPrice == msg.value, "Animal: incorrect hatch fee");
    ...
    emit hatched(msg.sender, _tokenId);
    return true;
  }
    ...
    returns (bool)
  {
    ...
    require(animal.inBreedMarket, "Animal: partner animal is not in breed market");
    require(animal.breedPrice == msg.value, "Animal: incorrect breed cost");
    require(canBreed(_tokenId), "Animal: your animal can't breed");
    require(canBreed(_partnerId), "Animal: partner animal can't breed");
    ...
    emit newEgg(msg.sender, _tokenId, _partnerId, id);
    return true;
  }
}
